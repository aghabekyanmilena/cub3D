...
/*   Updated: 2025/09/21 18:10:00 by atseruny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_wall_size(t_ray *ray, t_player *player, t_wall *wall)
{
	...
	wall_x -= floor((wall_x));
	wall->tex_x = (int)(wall_x * (double)texh);
	if (ray->side == 0 && ray->ray_dir_x > 0)
		wall->tex_x = texh - wall->tex_x - 1;
	if (ray->side == 1 && ray->ray_dir_y < 0)
		wall->tex_x = texh - wall->tex_x - 1;
	wall->step = 1.0 * texh / wall->line_height;
...
void	draw_wall(t_config *config, t_wall *wall, int x, int y)
{
	...
	{
		wall->tex_y = (int)wall->tex_pos & (texh - 1);
